        ...
        for (let j = 0; j < BOARD_SIZE; j++) {
            ...
            col.addEventListener("click", function() {
                if (!playerXName || !playerOName) {
                    ...
                    messageDiv.textContent = "Please enter both players' names to start the game.";
                ...
                if (!col.innerHTML && !boardState[i][j]) {
                    ...
                    if (checkWinner(i, j)) {
                        document.getElementById("game-result").innerHTML = `${currentPlayer === "X" ? playerXName : playerOName} wins!`;
                        saveGameHistory(currentPlayer === "X" ? playerXName : playerOName);
                    } else if (isBoardFull()) {
